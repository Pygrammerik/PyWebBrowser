...
def add_to_history(title, url, history_file):
    ...
    history = history[-500:]  # Ограничиваем историю 500 записями
    with open(history_file, "w", encoding="utf-8") as f:
...
class ProfileManagerDialog(QDialog):
    def __init__(self, parent=None):
        ...
        logo = QLabel("PyWeb Browser", self)
...
class SplashScreen(QSplashScreen):
    def __init__(self):
        ...
        self.timer.start(16)
        self.title = QLabel("PyWeb Browser", self)
...
class MainWindow(QMainWindow):
    def __init__(self, incognito_window=False):
        ...
        if self.incognito_window:
            self.setWindowTitle("PyWeb Browser — Инкогнито")
            ...
            settings_dir = os.path.join(self.profile_path, "settings")
            if not os.path.exists(settings_dir):
                os.makedirs(settings_dir)
            self.HISTORY_FILE = os.path.join(settings_dir, "history.json")
            self.QUICKLINKS_FILE = os.path.join(settings_dir, "quicklinks.json")
            ...
            if not os.path.exists(self.HISTORY_FILE):
                with open(self.HISTORY_FILE, "w", encoding="utf-8") as f:
                    json.dump([], f)
            if not os.path.exists(self.QUICKLINKS_FILE):
                with open(self.QUICKLINKS_FILE, "w", encoding="utf-8") as f:
                    json.dump([], f)
        self.setWindowTitle("PyWeb Browser - v2.0")
        self.setWindowIcon(QIcon("assets/logotip.png"))
